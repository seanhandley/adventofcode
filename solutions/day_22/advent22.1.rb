...
Player = Struct.new(:name, :hp, :armor, :mana, :damage, :mana_spent)

@boss   = Player.new("Boss", 55, 0, 0, 8, 0)
@wizard = Player.new("Wizard", 50, 0, 500, 0, 0)

Spell = Struct.new(:name, :mana, :damage, :armor, :heal, :turns, :recharge)

@spells = [
  Spell.new("Magic Missile", 53,  4, 0, 0, 0,   0),
  Spell.new("Drain",         73,  2, 0, 2, 0,   0),
  Spell.new("Shield",        113, 0, 7, 0, 6,   0),
  Spell.new("Poison",        173, 3, 0, 0, 6,   0),
  Spell.new("Recharge",      229, 0, 0, 0, 5, 101),
]

def fight(wizard, boss, spells)
  @spells_in_effect = []
  play = 1
  while(wizard.hp > 0 && boss.hp > 0)
    ...
    wizard.mana_spent += spell.mana
    raise "Mana expired :-(" if wizard.mana < 0
    # boss retaliates
    if boss.hp > 0
      wizard.hp -= [(boss.damage - wizard.armor), 1].max
    end
    debug boss
    debug wizard
  end
...
casts.each do |perm|
  wizard, boss = fight(@wizard.dup, @boss.dup, perm.map{|i| @spells[i].dup})
  if wizard.hp > boss.hp
    if wizard.mana_spent < winning_cost
      winning_cost = wizard.mana_spent
      p winning_cost
    end
  end
end
